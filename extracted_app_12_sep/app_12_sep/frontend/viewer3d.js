...
class Vcad3DViewer {
  ...
  onPointerDown(event) {
    ...
    const visibleMeshes = Array.from(this.meshMap.values()).filter(m => m.visible && m.userData.data.type !== 'SLAB' && m.userData.data.type !== 'GROUND_LAND');
    const intersects = this.raycaster.intersectObjects(visibleMeshes);

    if (intersects.length > 0) {
      // Prioritize room/flat cadastral units over walls so clicking inside a room selects the room
      const roomHit = intersects.find(i => i.object.userData && i.object.userData.data && i.object.userData.data.type !== 'WALL');
      const topHit = roomHit ? roomHit.object : intersects[0].object;
